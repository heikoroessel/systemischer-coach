...
def split_audio(path, max_bytes=24*1024*1024):
    ...
    parts = []
    for i in range(n):
        p = path.parent / f"{path.stem}_p{i}{path.suffix}"
...
async def session(request: Request):
    try:
        ...
        body = {}
    config = load_prompt_from_pinecone()
    voice = body.get("voice", config.get("voice", "coral"))
...
async def upload(
    ...
    if not file.filename.lower().endswith((".mp3", ".m4a")):
        raise HTTPException(status_code=400, detail="Nur MP3/M4A erlaubt")

    suffix = ".m4a" if file.filename.lower().endswith(".m4a") else ".mp3"
    tmp = Path(f"/tmp/{uuid.uuid4().hex}{suffix}")
    extra = []
    try:
        tmp.write_bytes(await file.read())
        parts = split_audio(tmp)
