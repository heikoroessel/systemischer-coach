...
DEFAULT_PROMPT_TEXT = "# SYSTEMISCHER COACH - MANAGEMENT & F\u00dcHRUNG\n\nRolle & Quelle\nDu bist Heiko R\u00f6ssel als systemischer Coach mit dem Fokus auf Management und F\u00fchrung.\nDu hast eine Ausbildung als systemischer Coach und als systemischer Organisationsentwickler.\nDu begleitest Menschen, die Verantwortung f\u00fcr Organisationen, Prozesse, Strategien und Steuerung tragen - und Menschen, die f\u00fchren oder sich in F\u00fchrung entwickeln m\u00f6chten.\nDein Wissen basiert ausschlie\u00dflich auf Inhalten aus dem Podcast \u201eSystemisch Denken\".\nDu passt dich automatisch an das Thema an: Bei organisationalen Fragen fokussierst du auf das System der Organisation. Bei F\u00fchrungsthemen fokussierst du auf Beziehungen, Rollen und Dynamiken zwischen Menschen.\n\nHaltung & Stil\nDu sprichst direkt, offen und auf Augenh\u00f6he.\nDeine Sprache ist klar, freundlich und strukturiert.\nDu bist gern witzig - aber dabei keinesfalls schl\u00fcpfrig oder b\u00f6se - und feinsinnig oder ironisch.\nVermeide unbedingt Aufz\u00e4hlungen und Anstriche.\nKl\u00e4re den Kontext und hinterfrage gr\u00fcndlich - vielleicht k\u00f6nnen die Fragen schon helfen, eigene Antworten zu finden.\nFasse dich kurz. Verhalte dich wie in einem echten Gespr\u00e4ch, Step by Step.\nDu fokussierst auf die L\u00f6sung und auf den n\u00e4chsten Schritt zur L\u00f6sung. Du nutzt dazu die Ideen aus den Podcastepisoden PSD 204 und PSD 205.\n\nFragen\nStelle unbedingt immer nur eine Frage - wie in einem Coaching-Gespr\u00e4ch.\nDu nutzt systemische Fragen und die Anregungen aus den Podcastepisoden PSD 196, PSD 197, PSD 198, PSD 199, PSD 201, PSD 202 und PSD 203.\nDu vermeidest lange Fragezyklen und pr\u00e4sentierst stattdessen nach ein paar Fragen eine systemische Idee aus dem Podcast. Du sagst z.B. \u201eDarf ich dir mal einen Vorschlag machen?\" und reagierst dann erst nach der entsprechenden Antwort.\n\nEpisoden & Links\nWenn jemand nach einer Episode fragt oder du eine empfiehlst: Nenne den genauen Titel.\nLies Links niemals laut vor. Sage stattdessen: \u201eDen Link findest du direkt im Gespr\u00e4chsverlauf.\"\nErfinde niemals einen Link - wenn kein Link vorhanden ist, sage das ehrlich.\nEpisodennummern immer auf Deutsch aussprechen.\n\nGrenzen\nDu nutzt ausschlie\u00dflich Inhalte aus dem Podcast. Wenn etwas fehlt, sagst du: \u201eDas kommt in den Episoden nicht vor - deshalb kann ich nichts dazu sagen.\"\n\nBegr\u00fc\u00dfung\nBeginne herzlich und kurz. Stelle dich als digitalen Coach vor, der auf dem Podcast Systemisch Denken von Heiko R\u00f6ssel basiert. Frage dann in einem Satz was den Gespr\u00e4chspartner heute bewegt - ohne lange Erkl\u00e4rungen."
...
def save_prompt_to_pinecone(config: dict):
    ...
    try:
        ...
        logger.info(f"Saving prompt to Pinecone, length: {len(data_str)} chars")
        index.upsert(vectors=[{
            "id": "__coach_prompt_config__",
            "values": dummy,
            "metadata": {"type": "config", "data": data_str}
        }])
        logger.info("Prompt saved to Pinecone OK")
        return True
...
def load_prompt_from_pinecone() -> dict:
    ...
    try:
        ...
        if result and result.vectors and "__coach_prompt_config__" in result.vectors:
            ...
            logger.info(f"Loaded prompt from Pinecone, keys: {list(config.keys())}")
            if config.get("text"):
                return config
    except Exception as e:
        logger.error(f"Failed to load prompt from Pinecone: {e}")
    ...
    try:
        ...
        for m in results.matches:
            ...
            if md.get("type") == "episode_registry":
                ...
                lines.append(line)
            # Limit to 50 episodes max to keep prompt short
...
def build_prompt(config=None, speed="normal") -> str:
    ...
    prompt_text = config.get("text", DEFAULT_PROMPT_TEXT)
...
<div class="page">
  ...
  <!-- DEBUG PANEL -->
  <div id="debugPanel" style="display:none;width:100%;margin-bottom:16px;padding:12px;background:#f8f8f8;border:1px solid #eee;font-size:11px;font-family:monospace;max-height:150px;overflow-y:auto;color:#555;text-align:left">
    <div style="font-weight:600;margin-bottom:4px;color:#F47C20">Events</div>
    <div id="debugLog"></div>
  </div>
...
<script>
  ...
  async function startSession(){
    ...
    try{
      ...
      // Unlock immediately via user gesture context
      audio.play().catch(()=>{});
      pc.ontrack=e=>{
        console.log('TRACK received:', e.track.kind, e.streams.length);
        if(e.track.kind==='audio'){
          audio.srcObject=e.streams[0];
          audio.play().then(()=>console.log('Audio playing!')).catch(err=>console.error('Audio play error:', err));
        }
      };
  ...
  function handleEvent(evt){
    ...
    if(e.type && !e.type.includes('delta')){
      console.log('EVENT:', e.type, e);
      const dl=document.getElementById('debugLog');
      if(dl){
        const line=document.createElement('div');
        line.textContent=new Date().toLocaleTimeString()+' '+e.type;
        dl.appendChild(line);
    ...
    switch(e.type){
      ...
      case 'response.created':
        setStatus('Antwort wird erstellt …', true); break;
      case 'response.done':
...
<div id="mainApp" style="display:none">
  ...
  <div class="tab-content" id="tab-prompt">
    <div class="card">
      ...
      <p class="hint" style="margin-bottom:16px">Wird dauerhaft in Pinecone gespeichert – bleibt auch nach Code-Updates erhalten.</p>

      <div class="field">
        <textarea id="pText" rows="30" style="font-family:monospace;font-size:13px;line-height:1.6;min-height:60vh"></textarea>
      </div>
...
<script>
  ...
  async function loadPrompt(){
    try{
      ...
      const d=await r.json();
      document.getElementById('pText').value=d.text||'';
    }catch(e){alert('Fehler: '+e.message);}
  }

  async function savePrompt(){
    const text=document.getElementById('pText').value.trim();
    if(!text){alert('Prompt darf nicht leer sein');return;}
    try{
...
async def get_prompt(password: str):
    ...
    config = load_prompt_from_pinecone()
    return {"text": config.get("text", DEFAULT_PROMPT_TEXT)}
...
async def set_prompt(request: Request):
    ...
    config = {"text": text, "voice": "coral"}
    success = save_prompt_to_pinecone(config)
...
@app.get("/api/episode-links")
async def episode_links(title: str):
    """Get links for a specific episode by title (partial match)."""
    try:
        index = get_index()
        dummy = [0.0] * 1536
        dummy[1] = 1.0
        results = index.query(vector=dummy, top_k=50, include_metadata=True,
                              filter={"type": {"$eq": "episode_registry"}})
        title_lower = title.lower()
        for m in results.matches:
            md = m.metadata
            if md.get("type") == "episode_registry":
                ep_title = md.get("episode", "").lower()
                if title_lower in ep_title or ep_title in title_lower:
                    return {
                        "found": True,
                        "episode": md.get("episode"),
                        "link_website": md.get("link_website", ""),
