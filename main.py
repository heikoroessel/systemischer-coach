...
def load_prompt_from_pinecone() -> dict:
    ...
    try:
        index = get_index()
        result = index.fetch(ids=["__coach_prompt_config__"], timeout=5)
...
VERFUEGBARE EPISODEN (nur Titel - keine Links erfinden!):
Wenn jemand einen Link will: sage "Den Link findest du im Gespraechsverlauf" - niemals einen Link erfinden.
{episode_list}
